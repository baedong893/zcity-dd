...
local material = Material("xyz/effects/infinitygauntlet/blackgoop")
...
function ENT:Draw()
	if self:GetPocketDimension():IsValid() and !self:GetPocketDimension():ShouldRenderInterior() then
		return
	end
	local mat = Matrix()
	...
	render.DrawQuad(self:LocalToWorld(Vector(-bounds.x,-bounds.y,bounds.z)),self:LocalToWorld(Vector(-bounds.x,bounds.y,bounds.z)),self:LocalToWorld(bounds),self:LocalToWorld(Vector(bounds.x,-bounds.y,bounds.z)),IG_StoneData[IG_STONE_REALITY].color)
end
