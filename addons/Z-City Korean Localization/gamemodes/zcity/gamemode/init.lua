...
function zb.RefreshRandomSpawns()
	getRandSpawn()
end
...
local function IsSpawnVector(pos)
	return isvector(pos)
end

local function NormalizeSpawnList(spawns)
	local out = {}

	for _, point in ipairs(spawns or {}) do
		local pos = isvector(point) and point or (istable(point) and (point.pos or point[1]))
		if IsSpawnVector(pos) then
			out[#out + 1] = pos
		end
	end
...
function zb:ChooseSeparatedTeamSpawns(team0spawns, team1spawns)
	team0spawns = NormalizeSpawnList(team0spawns)
	team1spawns = NormalizeSpawnList(team1spawns)

	if #team0spawns <= 0 then
		team0spawns = NormalizeSpawnList(spawners)
	end

	if #team1spawns <= 0 then
		team1spawns = NormalizeSpawnList(spawners)
	end

	if #team0spawns <= 0 or #team1spawns <= 0 then return end

	local best0, best1
	local bestDist = -1

	for _, pos0 in ipairs(team0spawns) do
		for _, pos1 in ipairs(team1spawns) do
			local dist = pos0:DistToSqr(pos1)
	...
	return best0 or table.Random(team0spawns), best1 or table.Random(team1spawns), bestDist
end
...
function zb:GetTeamSpawn(ply)
	...
	if round and round.DisableSeparatedTeamSpawns then
	...
	if not zb.teamSpawnPair then
		local spawn0, spawn1, dist = zb:ChooseSeparatedTeamSpawns(team0spawns, team1spawns)
		zb.teamSpawnPair = {
			[0] = spawn0,
			[1] = spawn1,
			dist = dist or 0
	...
	if team_ == 0 then
		if !zb.tspawn then
			zb.tspawn = zb.teamSpawnPair and zb.teamSpawnPair[0] or zb:GetRandomSpawn()
			pos = zb.tspawn
		else
		...
		if !zb.ctspawn then
			zb.ctspawn = zb.teamSpawnPair and zb.teamSpawnPair[1] or zb:GetRandomSpawn()
			pos = zb.ctspawn
		else
	...
	ErrorNoHalt("팀 스폰 지점을 찾을 수 없습니다. 잘못된 팀입니다.")
...
net.Receive("ZB_SpecMode",function(len,ply)
	...
	if (ply.ZBNextSpecModeRequest or 0) > CurTime() then return end
	ply.ZBNextSpecModeRequest = CurTime() + 1

	local wantsSpectator = net.ReadBool()
	...
	if wantsSpectator then
		...
		PrintMessage(HUD_PRINTTALK,ply:Name().." 님이 관전자로 참가했습니다.")
		return
	end

	if ply:Team() ~= TEAM_SPECTATOR then return end

	local playingTeam = zb and zb.BalancedChoice and zb:BalancedChoice(0, 1) or 1
	ply:SetTeam(playingTeam)
	PrintMessage(HUD_PRINTTALK,ply:Name().." 님이 플레이어로 참가했습니다.")
end)
