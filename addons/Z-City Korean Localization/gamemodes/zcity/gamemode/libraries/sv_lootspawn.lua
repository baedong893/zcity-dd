...
local spawns = {}
local spawnCacheReady = false

local function BuildSpawnCache(forceGenerate)
	local baseSpawns = {}

	for _, ent in pairs(ents.FindByClass("info_*")) do
		baseSpawns[#baseSpawns + 1] = ent:GetPos()
	end

	for _, area in pairs(navmesh.GetAllNavAreas()) do
		if area:IsUnderwater() then continue end
		baseSpawns[#baseSpawns + 1] = area:GetCenter()
	end

	if #baseSpawns <= 0 then
		spawnCacheReady = false
		return false
	end

	-- RandomSpawns is already persisted per map. Loading those points avoids
	-- repeating up to 1,000 traces during every server/map startup.
	if not forceGenerate then
		local storedSpawns = zb.TranslatePointsToVectors(zb.GetMapPoints("RandomSpawns"))
		if #storedSpawns > 0 then
			spawns = {}
			table.Add(spawns, baseSpawns)
			table.Add(spawns, storedSpawns)
			spawnCacheReady = true
			return true
		end
	end

	local generationPoints = {}
	table.CopyFromTo(baseSpawns, generationPoints)

	local generatedSpawns = MakeRandomSpawns(generationPoints, 0, 500, {})
	local savedPoints = zb.TranslateVectorsToPoints(generatedSpawns)

	spawns = {}
	table.Add(spawns, baseSpawns)
	table.Add(spawns, generatedSpawns)
	zb.SaveMapPoints("RandomSpawns", savedPoints)

	spawnCacheReady = true
	return true
end

local function EnsureSpawnCache()
	if spawnCacheReady and #spawns > 0 then return true end
	return BuildSpawnCache()
end

-- Nav areas and map spawn entities do not change between round cleanups. Build
-- this expensive cache once per map instead of doing 500 traces every round.
hook.Add("InitPostEntity", "ZCityBuildLootSpawnCache", function()
	EnsureSpawnCache()
end)

-- Explicit rebuild entry point for map-point/editor tools.
function zb.RebuildLootSpawnCache()
	spawnCacheReady = false
	return BuildSpawnCache(true)
end
