...
function zb.SaveMapPoints( pointGroup, pointsData ) -- Сохранаяет все точки в группе
    ...
    local map = game.GetMap()
    local cachedPoints = {}
    table.CopyFromTo(pointsData or {}, cachedPoints)
    zb.Points[pointGroup].Points = cachedPoints

    if pointGroup == "RandomSpawns" and zb.InvalidateWorldSizeCache then
        zb.InvalidateWorldSizeCache()
    end
...
local tdmCheckpointsCachedMap

function zb.tdm_checkpoints(forceRebuild)
    local currentMap = game.GetMap()
    if not forceRebuild and tdmCheckpointsCachedMap == currentMap then return end
    ...
    -- Map entities and their spawn coordinates are static across round cleanup.
    -- Keep the generated groups in memory until the map actually changes.
    tdmCheckpointsCachedMap = currentMap
end
