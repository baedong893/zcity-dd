...
if SERVER then
    ...
    net.Receive( "get_svPData", function( len, ply )
        ...
        local now = CurTime()
        if now >= (ply.ZBStatRequestWindow or 0) then
            ply.ZBStatRequestWindow = now + 1
            ply.ZBStatRequestCount = 0
        end

        ply.ZBStatRequestCount = (ply.ZBStatRequestCount or 0) + 1
        if ply.ZBStatRequestCount > 10 then return end

        local ent = net.ReadEntity()
        local dataName = net.ReadString()
        local getterName = allowedStatGetters[dataName]
        if not getterName or not IsValid(ent) or not ent:IsPlayer() then return end

        local getter = ent[getterName]
        if not isfunction(getter) then return end

        local ok, value = pcall(getter, ent)
    ...
    net.Receive( "get_svPData", function()
        ...
        if not allowedStatGetters[dataName] or not IsValid(ent) or not ent:IsPlayer() then return end
