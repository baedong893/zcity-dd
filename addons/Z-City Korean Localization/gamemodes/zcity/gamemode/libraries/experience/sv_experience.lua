...
local function EnsurePlayerInstance(ply)
    if not IsValid(ply) or not ply:IsPlayer() then return experienceDefaults end

    local steamID64 = ply:SteamID64()
    local instance = zb.Experience.PlayerInstances[steamID64]
    if not istable(instance) then
        instance = {}
        zb.Experience.PlayerInstances[steamID64] = instance
    end

    for key, fallback in pairs(experienceDefaults) do
        instance[key] = tonumber(instance[key]) or fallback
    end
...
function plyMeta:GetExp()
    ...
    return math.Round(EnsurePlayerInstance(self).experience)
...
function plyMeta:GetSkill()
    ...
    return EnsurePlayerInstance(self).skill
...
function plyMeta:GetDeaths()
    ...
    return EnsurePlayerInstance(self).deaths
...
function plyMeta:GetKills()
    ...
    return EnsurePlayerInstance(self).kills
...
function plyMeta:GetSuicides( ammout )
    ...
    return EnsurePlayerInstance(self).suicides
...
net.Receive("zb_xp_get",function(len,ply)
    ...
    if (ply.ZBNextExperienceRequest or 0) > CurTime() then return end
    ply.ZBNextExperienceRequest = CurTime() + 0.25
    ...
    net.Start("zb_xp_get")
        ...
        net.WriteInt( math.Clamp(get_ply:GetExp(), -2147483648, 2147483647), 32 )
    net.Send(ply)
