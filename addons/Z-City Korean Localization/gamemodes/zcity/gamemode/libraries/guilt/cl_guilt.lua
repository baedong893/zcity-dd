...
net.Receive("get_karma",function(len)
    ...
    for id,karma in pairs(tbl) do
        printTbl = printTbl.."\t"..(Player(id):Name().."님의 카르마: "..math.Round(karma,2)).."\n"
    end
...
local function harmdone(harm)
    if harm >= 9 then
        return "당신을 살해했습니다."
    elseif harm >= 5 then
        return "당신을 거의 죽게 만들었습니다."
    elseif harm >= 2 then
        return "당신에게 중상을 입혔습니다."
    elseif harm >= 1 then
        return "당신에게 경상을 입혔습니다."
    else
        return "당신에게 약간의 피해를 주었습니다."
    end
end
...
OpenMenu = function(tbl)
    ...
    for ply, harm in pairs(tbl) do
        ...
        but.name = ply:Name()
        but.harm = harm
        local txt = but.name.."님을 용서하시겠습니까? 용서 시 "..math.Round(but.harm,1).."만큼의 카르마가 회복됩니다."
        local clr = 255
