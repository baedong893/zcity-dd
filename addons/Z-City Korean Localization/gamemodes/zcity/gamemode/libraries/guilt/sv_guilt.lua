...
local function ClampKarma(value)
    return math.Clamp(tonumber(value) or 100, -60, zb.MaxKarma or 100)
end
	...
	local query = mysql:Select("zb_guilt")
		...
		query:Callback(function(result)
			if (IsValid(ply) and istable(result) and #result > 0 and result[1].value) then
                ...
                zb.GuiltSQL.PlayerInstances[steamID64].value = ClampKarma(result[1].value)
                ...
                if zb.GuiltSQL.PlayerInstances[steamID64].value < 0 then
                    ...
                    timer.Simple(0, function()
                        if IsValid(ply) then
                            ply:ChatPrint("카르마가 너무 낮아 10으로 초기화되었습니다: " .. math.Round( karma, 0 ))
                        end
                    end)
                end
			else
...
function plyMeta:guilt_GetValue()
    ...
    return ClampKarma(zb.GuiltSQL.PlayerInstances[self:SteamID64()] and zb.GuiltSQL.PlayerInstances[self:SteamID64()].value or 100)
...
local function ResetRoundKarmaStats(ply)
    ply.ZB_KarmaRoundParticipated = false
    ply.ZB_KarmaRoundTeamShots = 0
    ply.ZB_KarmaRoundTeamKilled = false
    ply.ZB_KarmaRoundTeamShotLast = {}
end

local function MarkRoundKarmaParticipation(ply)
    if IsValid(ply) and ply:IsPlayer() then
        ply.ZB_KarmaRoundParticipated = true
    end
end

local function CountRoundTeamShot(attacker, victim)
    if not IsValid(attacker) or not attacker:IsPlayer() then return end
    if not IsValid(victim) or not victim:IsPlayer() then return end

    attacker.ZB_KarmaRoundTeamShotLast = attacker.ZB_KarmaRoundTeamShotLast or {}

    local key = victim:SteamID()
    local last = attacker.ZB_KarmaRoundTeamShotLast[key] or 0

    if last + 1 > CurTime() then return end

    attacker.ZB_KarmaRoundTeamShotLast[key] = CurTime()
    attacker.ZB_KarmaRoundTeamShots = (attacker.ZB_KarmaRoundTeamShots or 0) + 1
end
...
hook.Add("HomigradDamage", "GuiltReg", function(ply, dmgInfo, hitgroup, ent, harm) 
    ...
    if not rnd or zb.ROUND_STATE ~= 1 then return end
	local karmaDisabled = zb.IsKarmaDisabledForCurrentRound and zb.IsKarmaDisabledForCurrentRound()
	...
	if not karmaDisabled then
		MarkRoundKarmaParticipation(Attacker)
		if Victim:IsPlayer() then MarkRoundKarmaParticipation(Victim) end

		local victimTeam = Victim:IsPlayer() and Victim:Team() or Victim.team
		if Victim:IsPlayer() and attackerTeam == victimTeam and Attacker ~= Victim then
			CountRoundTeamShot(Attacker, Victim)

			if newharm >= maxharm and oldharmdone < newharm then
				Attacker.ZB_KarmaRoundTeamKilled = true
			end
		end
	end

    if hg_developer:GetBool() then
        Attacker:ChatPrint("입힌 피해량: "..math.Round(harm,3))
        Attacker:ChatPrint("총 가해량: "..math.Round(amt,3))
        Attacker:ChatPrint("누적 피해량: "..math.Round(newharm,3))
        Attacker:ChatPrint("누적 죄책감: "..math.Round(amt * 60,3))
        Attacker:ChatPrint(" ")
    end
	...
	if karmaDisabled or GetConVar("zb_dev"):GetBool() then return end
    ...
    if shouldBanGuilt and Attacker.Guilt >= 100 then
        Attacker.Karma = math.min(Attacker.Karma or 100, 10)
        Attacker:SetNetVar("Karma", Attacker.Karma)
        PrintMessage(HUD_PRINTTALK, "플레이어 " .. Attacker:Name() .. "님이 팀 데미지로 인해 카르마 패널티를 받았습니다.")
    end
    ...
    if Attacker.Karma <= 0 then
        ...
        timer.Create("simplewaitforkarmadrop"..Attacker:EntIndex(), 0, 1, function()
            if IsValid(Attacker) then
                Attacker.organism.start_shaking = CurTime() + 15
                hg.StunPlayer(Attacker)
            end

            PrintMessage(HUD_PRINTTALK, "플레이어 " .. name .. "님이 카르마 수치 미달로 인해 패널티를 받았습니다.")
        end)
    end
end)
...
hook.Add("Player Think", "karmagain", function(ply)
	if zb.IsKarmaDisabledForCurrentRound and zb.IsKarmaDisabledForCurrentRound() then return end
...
hook.Add("Org Think", "Its_Karma_Bro",function(owner, org, timeValue)
	if zb.IsKarmaDisabledForCurrentRound and zb.IsKarmaDisabledForCurrentRound() then
		org.start_shaking = nil
		return
	end
...
hook.Add("ZB_EndRound","savevalues",function()
    ...
    for i,ply in player.Iterator() do
		if karmaDisabled then
			ResetRoundKarmaStats(ply)
			continue
		end

        local participated = ply.ZB_KarmaRoundParticipated or (ply:Alive() and ply:Team() ~= TEAM_SPECTATOR)
        local teamShots = ply.ZB_KarmaRoundTeamShots or 0
        local bonus = 0

        if participated and not ply.ZB_KarmaRoundTeamKilled then
            if teamShots <= 0 then
                bonus = 10
            elseif teamShots == 1 then
                bonus = 5
            else
                bonus = 1
            end
        ...
        if bonus > 0 then
            ply.Karma = math.Clamp((ply.Karma or 100) + bonus, 0, zb.MaxKarma)
            ply:SetNetVar("Karma", ply.Karma)
        end
...
concommand.Add("hg_setkarma",function(ply,cmd,args)
    ...
    newply.Karma = ClampKarma(lenargs > 1 and args[2] or args[1])
...
hook.Add("Player Spawn", "GuiltKnown",function(ply)
	if zb.IsKarmaDisabledForCurrentRound and zb.IsKarmaDisabledForCurrentRound() then return end
    if ply.Karma then
        local prefix = ZCLang and ZCLang.PlayerT and ZCLang.PlayerT(ply, "chat_current_karma", "Current karma: ") or "Current karma: "
        ply:ChatPrint(prefix .. tostring(math.Round(ply.Karma)))
    end
end)
...
hook.Add("ZC_SomeoneGetFallBy","IdiotsMustBeKilled",function(Attacker,Victim)
	...
	if (zb.IsKarmaDisabledForCurrentRound and zb.IsKarmaDisabledForCurrentRound()) or GetConVar("zb_dev"):GetBool() then return end
