...
function MODE:HUDPaint()
	if zb.ROUND_START + 5 > CurTime() then
		...
		if IsValid(dmmusic) then
			...
			local maxVolume = ply:Alive() and organism.otrub and 0.1 or 1 + math.min((organism.adrenaline or 0) * 25, 2)
    ...
    draw.SimpleText(T("mode_name_superfighters", "Superfighters 3D"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local Rolename = T("role_superfighter", fighter.name)
    ...
    local Objective = T("mode_sfd_objective", fighter.objective)
