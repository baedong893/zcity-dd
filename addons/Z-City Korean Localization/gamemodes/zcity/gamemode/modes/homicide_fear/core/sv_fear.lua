...
local crysound
local hooks = {}
local ambientCorpseCount = 8
local ambientCorpseModels = {
...
local function TraceFearFloor(pos)
	...
	if not tr.Hit or tr.HitNormal.z < 0.55 then return end
	return tr.HitPos + Vector(0, 0, 18)
end

local function FindFearAmbientCorpsePosition()
	if navmesh and navmesh.GetAllNavAreas then
		local areas = navmesh.GetAllNavAreas()

		for _ = 1, 28 do
			local area = table.Random(areas or {})
			if not area then break end

			local pos = TraceFearFloor(area:GetRandomPoint())
	...
	for _ = 1, 32 do
		...
		if not IsValid(spawn) then break end

		local angle = math.rad(math.Rand(0, 360))
		local dist = math.Rand(180, 900)
		local pos = TraceFearFloor(spawn:GetPos() + Vector(math.cos(angle) * dist, math.sin(angle) * dist, 96))
		if pos then return pos end
	end
end

function MODE:ClearAmbientCorpses()
	for _, ent in ipairs(ents.FindByClass("prop_ragdoll")) do
		if ent.ZC_FearAmbientCorpse then
			ent:Remove()
		end
	end
end

function MODE:SpawnAmbientCorpses()
	self:ClearAmbientCorpses()

	for i = 1, ambientCorpseCount do
		local pos = FindFearAmbientCorpsePosition()
		if not pos then continue end

		local corpse = ents.Create("prop_ragdoll")
		if not IsValid(corpse) then continue end

		corpse:SetModel(table.Random(ambientCorpseModels))
		corpse:SetPos(pos)
		corpse:SetAngles(Angle(0, math.Rand(0, 360), 0))
		corpse:Spawn()
		corpse:Activate()
		corpse:SetCollisionGroup(COLLISION_GROUP_DEBRIS)
		corpse.ZC_FearAmbientCorpse = true

		local bones = corpse:GetPhysicsObjectCount()
		for bone = 0, bones - 1 do
			local phys = corpse:GetPhysicsObjectNum(bone)
			if IsValid(phys) then
				phys:Wake()
				phys:SetVelocity(Vector(math.Rand(-35, 35), math.Rand(-35, 35), math.Rand(-25, 10)))
			end
		end
	end
end

local function FearBloodBurstAt(pos)
	for _ = 1, 8 do
		local effect = EffectData()
		effect:SetOrigin(pos + Vector(math.Rand(-12, 12), math.Rand(-12, 12), math.Rand(-12, 12)))
		effect:SetNormal(VectorRand():GetNormalized())
		effect:SetScale(math.Rand(1.4, 3))
		util.Effect("BloodImpact", effect, true, true)
	end
end

local function HideFearRagdollBone(ragdoll, boneName)
	local bone = ragdoll:LookupBone(boneName)
	if bone == nil then return end

	local pos = select(1, ragdoll:GetBonePosition(bone)) or ragdoll:WorldSpaceCenter()
	ragdoll:ManipulateBoneScale(bone, Vector(0.01, 0.01, 0.01))
	FearBloodBurstAt(pos)
end

function MODE:BurstFearCorpse(ragdoll)
	if not IsValid(ragdoll) or ragdoll.ZC_FearCorpseBurst then return end

	ragdoll.ZC_FearCorpseBurst = true

	for _, boneName in ipairs(fearCorpseBurstBones) do
		HideFearRagdollBone(ragdoll, boneName)
	end

	ragdoll:EmitSound("player/zombie_head_explode_0" .. math.random(1, 6) .. ".wav", 90, math.random(75, 95), 1)
	util.ScreenShake(ragdoll:GetPos(), 9, 20, 0.8, 850)
end

function MODE:TriggerFearDefeatBurst()
	self.saved = self.saved or {}
	if self.saved.FearDefeatBurstDone then return end
	self.saved.FearDefeatBurstDone = true

	for _, ply in player.Iterator() do
		ChatT(ply, "sv_fear_defeat", "모든 생존자가 사망했습니다. 공포에 잠식되어 패배했습니다.")
	end

	for _, ragdoll in ipairs(ents.FindByClass("prop_ragdoll")) do
		self:BurstFearCorpse(ragdoll)
	end
end
...
function MODE:Intermission()
	if not self:IsFearRound() then return end
	...
	self.saved.FearDefeatBurstDone = false
	...
	for k, ply in player.Iterator() do
		if(MODE.ShouldStartRoleRound())then
			net.Start("HMCD_RoundStart")	--; TODO Structure description
				...
				-- 공포 모드는 역할 선택 단계가 없으므로 즉시 시작 완료 상태로 보낸다.
				-- false를 보내면 공통 Homicide HUD가 목적을 계속 "라운드 시작 중"으로 덮는다.
				net.WriteBool(true)	--; Round Started
	...
	self:CreateTimer("FearAmbientCorpses", 1, 1, function()
		self:SpawnAmbientCorpses()
	end)
end

function MODE:ShouldRoundEnd()
	local noSurvivors = #zb:CheckAlive() == 0
	if noSurvivors then
		self:TriggerFearDefeatBurst()
	end
...
function MODE:EndRound()
	if not self:IsFearRound() and not (zb and zb.CROUND_MAIN == self.name) then return end
