...
function MODE:IsFearRound()
	local mode = CurrentRound and CurrentRound()
	return mode and mode.name == self.name
end
...
function MODE:CreateTimer(name, delay, repetitions, func)
	...
	timer.Create(name, delay, repetitions, function(...)
		if not self:IsFearRound() then
			timer.Remove(name)
			if self.saved and self.saved.Timers then
				self.saved.Timers[name] = nil
			end
