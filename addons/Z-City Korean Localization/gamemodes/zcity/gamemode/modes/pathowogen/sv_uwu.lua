...
function MODE:EndRound()
	PrintMessage(HUD_PRINTTALK, "라운드가 종료되었습니다.")
...
function MODE:RoundStart()
	...
	self.saved.ResponseTeamArrived = false
...
function MODE:SpawnDeltaSquad(count)
	...
	net.Start("zb_commandertransmit")
		net.WriteString("잊지 마라. 너희는 이 지역에 남아있는 생존자들을 구출하기 위해 온 것이다. 적어도 한 명이라도 데려오지 못한다면 기지로 복귀할 생각조차 하지 마라.")
...
function MODE:SpawnSquadHelicopter()
	...
	if #heliPoints > 0 then
		self:CreateTimer("UWUCopter", math.random(240, 300), 1, function()
			...
			if #squad == 0 then
                self:BroadcastCommander("미안하다, 탈출 수단은 오지 않는다. 너희는 끝났다. 차라리 자결하는 게 나을 거다.")
            else
                self:BroadcastCommander("탈출 헬기가 현재 작전 지역에 도착했다. 헬기를 찾아 어떻게든 신호를 보내라.")
...
function MODE:InitiateCQExtraction()
	...
	if spawnPoint then
		self:CreateTimer("CQExtract", math.random(120, 180), 1, function()
			local squad = self:SpawnPoliceResponse(4)

            print("경찰 대응팀:")
            PrintTable(squad)

            if #squad == 0 then
                self:BroadcastCommander("경찰 대응팀 투입에 실패했다. 지원 가능한 인원이 없다.")
            else
                self:BroadcastCommander("경찰 대응팀이 도착했다. 행운을 빌지.")
...
function MODE:SpawnPoliceResponse(count)
	local spawned = 0
	local spawnedPlayers = {}
	...
	for _, ply in RandomPairs(player.GetAll()) do
		...
		zb.GiveRole(ply, "Police Response", Color(10, 10, 100))
		self:GetRandomSpawn(ply)
...
function MODE:RoundThink()
	if self.saved.ResponseTeamArrived then return end
	if (CurTime() - (zb.ROUND_BEGIN or 0)) < 180 then return end

	self.saved.ResponseTeamArrived = true
	local police = self:SpawnPoliceResponse(4)

	if #police > 0 then
		PrintMessage(HUD_PRINTTALK, "경찰 대응팀이 도착했습니다. 감염 구역이 봉쇄되었습니다.")
	else
		PrintMessage(HUD_PRINTTALK, "경찰 대응팀이 출동하려 했지만 투입 가능한 인원이 없습니다.")
	end
...
function MODE:CanPlayerEnterVehicle(ply, ent) -- damdn i forgot about the broken mode hooks lmao
	if ply.PlayerClassName == "furry" then
		if (ply.cantdrivecd or 0) < CurTime() then
			ply:Notify("어... 운전하는 방법 모르는뎅.", 0, "idkdrive", 0)
...
function MODE:SpawnGlideHelicopter()
		...
		if extracted then
			for _, v in ipairs(players) do
				...
				if !self.saved.ContractorEscapee then
                    ...
                    self:BroadcastContractor("누군가 방금 탈출했다! 임무는 실패다! 계약을 해지한다.")
                end
            end
            ...
            timer.Simple(5, function()
                self:BroadcastCommander("탈출 헬기가 방금 지역을 떠났다. 미안하군 친구, 자네는 뒤처진 것 같아.")
            end)
        end
    end
    ...
    heli:CallOnRemove("CommanderReply", function()
        ...
        timer.Simple(5, function()
            self:BroadcastCommander("헬기와의 교신이 끊겼다... 제기랄...")
        end)
    end)
