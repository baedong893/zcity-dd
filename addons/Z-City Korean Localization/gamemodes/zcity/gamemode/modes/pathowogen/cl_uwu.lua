...
function MODE:PostDrawTranslucentRenderables(depth, skybox, skybox2)
	...
	cam.Start3D2D( pos + UpVector, angle, scale)
		local label = T("pathowogen_extraction_point", "[Extraction Point]")
		draw.SimpleText(label, "ZB_ScrappersHumongous", 10, 10, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText(label, "ZB_ScrappersHumongous", 0, 0, ExtractionColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
...
function MODE:HUDPaint()
	...
	if extraction then
		...
		if time > 0 then
			local label = T("pathowogen_extraction_countdown", "Extraction in: ") .. time
            draw.SimpleText(label, "ZB_ScrappersMedium", sw * 0.01 + 2, sh * 0.97 + 2, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText(label, "ZB_ScrappersMedium", sw * 0.01, sh * 0.97, ExtractionColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		end
	end
end
