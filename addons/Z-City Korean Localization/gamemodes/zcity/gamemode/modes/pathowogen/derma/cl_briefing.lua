...
function PANEL:Init()
    ...
    self.dialogue:SetText("현재 작전 지역에 정체불명의 변종 바이러스(Pathowogen)가 창궐하고 있다. 추후 통지가 있을 때까지 해당 구역은 즉시 격리 조치되며, 그동안 우리는 너희를 탈출시킬 방법을 찾아보겠다.", 2)
    timer.Simple(15, function()
        if !IsValid(self) then return end
        self.dialogue:SetText("몸 조심해라. 이상.", 2)
