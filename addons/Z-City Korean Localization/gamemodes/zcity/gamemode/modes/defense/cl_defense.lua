...
net.Receive("npc_defense_start",function()
    ...
    hook.Run("ZB_DefenseRoundStarted")
end)

local teams = {
	[1] = {
        objective = "콤바인의 공격으로부터 기지를 방어하십시오.",
        name = "난민",
...
function MODE:HUDPaint()
	if NextWave_Time > CurTime() - 5 then
		...
		draw.SimpleText(T("defense_next_wave", "Next wave in: ") .. time.m .. ":" .. time.s, "ZB_HomicideMedium", sw * 0.5, sh * (0.9 + timePos), Color(87,146,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
    ...
    draw.SimpleText("ZBattle | " .. T("mode_name_defense", "NPC Defense"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	
    local playerRole = lply:GetNWString("PlayerRole", "Refugee") 
    local roleKey = ZCLang and ZCLang.RoleTextKeys and ZCLang.RoleTextKeys[playerRole]
    local roleColor = teams[team_].color1
    roleColor.a = 255 * fade
    draw.SimpleText(T("common_your_role", "Your role: ") .. T(roleKey, playerRole), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.5, roleColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

    local objective = T("objective_defense_hold", teams[team_].objective)
...
closebutton.Paint = function(self,w,h)
    ...
    hmcdEndMenu.PaintOver = function(self,w,h)
        ...
        surface.SetTextColor(col.r,col.g,col.b,col.a)
        local lengthX, lengthY = surface.GetTextSize("플레이어:")
        surface.SetTextPos(w / 2 - lengthX/2,20)
        surface.DrawText("플레이어:")
    end
...
local lengthX, lengthY = surface.GetTextSize( ply:GetPlayerName() or "퇴장함..." )
            ...
            surface.DrawText((ply:Name() .. (not ply:Alive() and " - 사망" or "")) or "퇴장함...")
...
local function CreateVoteMenu()
    ...
    voteMenu.Paint = function(self, w, h)
        ...
        draw.SimpleText("게임 모드 선택", "Defense_Title", w / 2, 28, Color(230, 230, 230), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("이번 라운드에 진행할 모드에 투표하세요", "Defense_Subtitle", w / 2, 55, Color(200, 200, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    timerPanel.Paint = function(self, w, h)
        ...
        draw.SimpleText("남은 시간:", "Defense_Stats", w / 2, 12, Color(200, 200, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText(timeLeft .. " 초", "Defense_Timer", w / 2, 32, timeColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    voteStatsPanel.Paint = function(self, w, h)
        ...
        draw.SimpleText("투표 통계:", "Defense_Stats", 20, 20, Color(200, 200, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText("총 투표수: " .. totalVotes, "Defense_Stats", w - 20, 20, Color(200, 200, 200), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
        

        draw.RoundedBox(5, 170, 15, 140, 40, Color(50, 100, 200, 150))
        draw.SimpleText("표준: " .. standardPercent .. "%", "Defense_Stats", 240, 33, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("(" .. (voteResults[1] or 0) .. "표)", "Defense_SmallText", 240, 48, Color(220, 220, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        

        draw.RoundedBox(5, 370, 15, 140, 40, Color(200, 100, 50, 150))
        draw.SimpleText("확장: " .. extendedPercent .. "%", "Defense_Stats", 440, 33, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("(" .. (voteResults[2] or 0) .. "표)", "Defense_SmallText", 440, 48, Color(220, 220, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        

        draw.RoundedBox(5, 570, 15, 140, 40, Color(50, 200, 50, 150))
        draw.SimpleText("좀비: " .. zombiePercent .. "%", "Defense_Stats", 640, 33, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("(" .. (voteResults[3] or 0) .. "표)", "Defense_SmallText", 640, 48, Color(220, 220, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    local modeDescriptions = {
        [1] = {
            title = "표준 모드",
            shortDesc = "클래식한 6개 웨이브의 콤바인 공격",
            longDesc = "전통적인 클래식 모드입니다.",
            color = Color(50, 100, 200),
            features = {
                "• 난이도가 상승하는 6개 웨이브의 콤바인 병사 공격",
                "• 일반 적 유형: 시민 보호국 및 콤바인 솔저",
                "• 마지막 웨이브에 정예 콤바인 솔저 포함",
                "• 신규 플레이어 및 표준 게임플레이 권장"
            }
        },
        [2] = {
            title = "확장 모드",
            shortDesc = "보스와 특수 적들이 등장하는 12개 웨이브",
            longDesc = "베테랑 플레이어를 위한 도전! 무자비한 유혈 사태와 잔혹함이 가득한 12개 웨이브입니다.",
            color = Color(200, 100, 50),
            features = {
                "• 12개 웨이브의 집중 전투",
                "• 특수 보스 웨이브 등장",
                "• 표준 모드보다 훨씬 높은 난이도 - 숙련자 전용",
                "• 터렛, 맨핵 및 기타 특수 적 유형 포함"
            }
        },
        [3] = {
            title = "좀비 모드",
            shortDesc = "6개 웨이브의 좀비 아포칼립스",
            longDesc = "콤바인 대신 다양한 좀비들이 등장하는 독특한 모드입니다.",
            color = Color(50, 200, 50),
            features = {
                "• 6개 웨이브의 좀비 군단",
                "• 콤바인과는 다른 독특한 도전 과제",
                "• 표준 모드보다 더 많은 수의 적 등장"
            }
        }
    }
    ...
    local function CreateModeButton(index, yPos)
        ...
        descPanel.Paint = function(self, w, h)
            ...
            if isDisabled then
                draw.SimpleText("이 모드는 현재 개발 중이며 곧 이용 가능합니다!",
...
MODE.HUDPaint = function(self)
    ...
    if showSelectedMode and CurTime() - selectedModeDisplayTime < selectedModeDisplayDuration then
        ...
        elseif currentSubMode == "EXTENDED" then
            ...
            description = "보스와 특수 적들이 등장하는 12개 웨이브"
...
local function DrawBossIncomingBanner()
    ...
    draw.SimpleText("보스 출현", "ZB_HomicideMediumLarge", centerX + 2, textY + 2, shadowColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    

    draw.SimpleText("보스 출현", "ZB_HomicideMediumLarge", centerX, textY, textColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    

    local infoY = y + headerHeight + (scaledHeight - headerHeight) / 2
    draw.SimpleText("강력한 적이 나타납니다. 전투를 준비하십시오!", "ZB_HomicideMedium", centerX, infoY, textColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
