...
function MODE:EndWave(cancelled)
    ...
    self.WaveCompleted = not cancelled
...
function MODE:EndVoting()
    ...
    timer.Simple(3, function()
        if CurrentRound() ~= self or zb.ROUND_STATE == 3 then return end
                   ...
                   class ~= "npc_bullseye_new" then
                    ...
                    if not isDead then
                        ...
                        if not ent.DefenseEntityID then
                            ...
                            print("[DEFENSE] 추적할 새 NPC 발견: " .. class)
                        end
