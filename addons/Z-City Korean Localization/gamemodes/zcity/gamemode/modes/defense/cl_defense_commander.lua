...
local MAX_ITEM_QUANTITY = 10

local function NotifyQuantityLimit()
    chat.AddText(COMMANDER_UI.PRIMARY_COLOR, "각 항목은 한 주문당 최대 10개까지 구매할 수 있습니다.")
...
function CreateCommanderMenu()
    ...
    commanderMenu.Paint = function(self, w, h)
        ...
        draw.SimpleText("지휘관 보급 청구", "CommanderTitle", w/2, 25, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

        draw.SimpleText("가용 포인트: " .. points, "CommanderText", w - 120, 25, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
    end
    ...
    cartPanel.Paint = function(self, w, h)
        ...
        draw.SimpleText("당신의 주문", "CommanderCategory", 10, 10, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_LEFT)
        
        local totalCost = CalculateCartCost()
        draw.SimpleText("총 비용: " .. totalCost .. " 포인트", "CommanderText", w - 10, 10, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_RIGHT)
    end
    ...
    local function UpdateCartDisplay()
        ...
        for i, item in ipairs(currentCart) do
            ...
            plusButton.DoClick = function()
                if (item.quantity or 1) >= MAX_ITEM_QUANTITY then
                    NotifyQuantityLimit()
                    return
                end
    ...
    orderButton.Paint = function(self, w, h)
        ...
        draw.SimpleText("주문하기", "CommanderText", w/2, h/2, COMMANDER_UI.TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    orderButton.DoClick = function()
        ...
        if totalCost > points then
            chat.AddText(COMMANDER_UI.PRIMARY_COLOR, "보급 포인트가 부족합니다!")
            surface.PlaySound("buttons/button10.wav")
            return
        end
    ...
    local function PopulateItems(category)
        ...
        for _, item in ipairs(availableItems[category]) do
            ...
            plusBtn.DoClick = function()
                local maxPossibleItems = math.min(math.floor(points / item.price), MAX_ITEM_QUANTITY)
                if quantity < maxPossibleItems then
                    quantity = quantity + 1
                    quantityLabel:SetText("x" .. quantity)
                    surface.PlaySound("ui/buttonclick.wav")
                elseif quantity >= MAX_ITEM_QUANTITY then
                    NotifyQuantityLimit()
                end
            end
            ...
            addToCartBtn.DoClick = function()
                if item.price * quantity > points then
                    chat.AddText(COMMANDER_UI.PRIMARY_COLOR, "이 항목을 사기 위한 보급 포인트가 부족합니다!")
                    surface.PlaySound("buttons/button10.wav")
                    return
                end
                ...
                if existingItemIndex then
                    local existingQuantity = currentCart[existingItemIndex].quantity or 1
                    if existingQuantity + quantity > MAX_ITEM_QUANTITY then
                        NotifyQuantityLimit()
                        return
                    end

                    currentCart[existingItemIndex].quantity = existingQuantity + quantity
                else
...
local COMMANDER_HINT = {
    ...
    text = [[지휘관으로서, 당신은 팀을 지원할 책임이 있습니다!  
    
Q 메뉴를 사용하여 병사들을 위한 장비와 지원을 주문하십시오.

매 웨이브가 끝날 때마다 보급 포인트를 얻습니다. 현명하게 사용하십시오!

당신의 팀은 당신의 리더십과 전술적 결정에 의존하고 있습니다!]]
}
...
timer.Create("CommanderHintChecker", 1, 0, function()
    ...
    if ply:GetNWString("PlayerRole") == "Commander" and ply:Alive() and not COMMANDER_HINT.shownThisRound then
        ShowCommanderHint()
        COMMANDER_HINT.shownThisRound = true
        print("[디펜스] 타이머에 의해 지휘관 힌트가 트리거됨")
    end
end)


hook.Add("ZB_DefenseRoundStarted", "ResetCommanderHintOnDefenseStart", function()
    COMMANDER_HINT.shownThisRound = false
    timer.Simple(2, CheckAndShowCommanderHint)
    print("[디펜스] 방어 시작 시 힌트 상태 초기화")
end)

net.Receive("npc_defense_prepphase", function()
    timer.Simple(1, CheckAndShowCommanderHint)
    print("[디펜스] 준비 단계에서 힌트 확인 중")
end)
