...
local teams = {
    [0] = {
        objective = "그루브파들을 전부 담구십시오",
        name = "블러즈 단원",
        color1 = Color(180, 0, 0),
        color2 = Color(180, 0, 0)
    },
    [1] = {
        objective = "블러즈파들을 전부 담구십시오",
        name = "그루브 단원",
        color1 = Color(0, 180, 0),
        color2 = Color(0, 180, 0)
    },
}
teams[0].objective = "Take out all Groove members."
teams[0].name = "Bloodz"
teams[1].objective = "Take out all Bloodz members."
teams[1].name = "Groove"
local lerpsnd = 0.3
...
function MODE:HUDPaint()
	...
	if timeBeforeSWAT > 0 and zb.ROUND_START + 10.5 < CurTime() then
		...
		local swatText = T("mode_gwars_swat_timer_suffix", " until SWAT arrival!")
		local w2, h2 = surface.GetTextSize("11:11:11" .. swatText)
		surface.SetTextPos(sw * 0.5 - w2 / 2, sh * 0.05)
		surface.DrawText(time)
		surface.SetTextPos(sw * 0.5 - w2 / 2 + w, sh * 0.05)
		surface.DrawText(swatText)
		//draw.SimpleText(" SWAT 대원 도착까지 남은 시간!", "timer_Font2", sw * 0.432, sh * 0.05, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    ...
    draw.SimpleText("ZBattle | " .. T("mode_name_gwars", "Gang Wars"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	local roleKey = team_ == 1 and "role_groove" or "role_bloodz"
	local objectiveKey = team_ == 1 and "mode_gwars_groove_obj" or "mode_gwars_bloodz_obj"
	...
	local Objective = T(objectiveKey, teams[team_].objective)
	...
	if hg.PluvTown.Active then
		...
		draw.SimpleText(T("mode_gwars_pluvtown_somewhere", "Somewhere in Pluv Town"), "ZB_ScrappersLarge", sw / 2, sh * 0.44 - ScreenScale(2), Color(0, 0, 0, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end
