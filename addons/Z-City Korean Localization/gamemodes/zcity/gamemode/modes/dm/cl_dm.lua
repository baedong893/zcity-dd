...
--
    ...
    draw.SimpleText(T("mode_dm_title", "Homicide | Deathmatch"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local Rolename = T("mode_dm_fighter", fighter.name)
    ...
    local Objective = T("mode_dm_objective", fighter.objective)
	...
	if hg.PluvTown.Active then
		...
		draw.SimpleText(T("pluvtown_madness", "Pluvtown Madness"), "ZB_ScrappersLarge", sw / 2, sh * 0.44 - ScreenScale(2), Color(0, 0, 0, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end
...
CreateEndMenu = function()
    ...
    hmcdEndMenu.PaintOver = function(self,w,h)
        ...
        local txt = (wonply and wonply:GetPlayerName() or "Nobody") .. " wins!"
	...
	for i,ply in player.Iterator() do
		...
		but.Paint = function(self,w,h)
			...
			surface.SetFont( "ZB_InterfaceMediumLarge" )
            local lengthX, lengthY = surface.GetTextSize( ply:GetPlayerName() or "알 수 없음" )
            
            surface.SetTextColor(0,0,0,255)
            surface.SetTextPos(w / 2 + 1,h/2 - lengthY/2 + 1)
            surface.DrawText(ply:GetPlayerName() or "알 수 없음")
            ...
            surface.DrawText(ply:Name() .. (ply.most_violent_player and " - MVP" or (not ply:Alive() and " - Dead" or "")))

            surface.SetFont( "ZB_InterfaceMediumLarge" )
            surface.SetTextColor(col.r,col.g,col.b,col.a)
            local lengthX, lengthY = surface.GetTextSize( ply:Frags() or "알 수 없음" )
            surface.SetTextPos(w - lengthX -15,h/2 - lengthY/2)
            surface.DrawText(ply:Frags() or "알 수 없음")
		end

		function but:DoClick()
			if ply:IsBot() then chat.AddText(Color(255,0,0), "봇은 프로필이 없습니다.") return end
