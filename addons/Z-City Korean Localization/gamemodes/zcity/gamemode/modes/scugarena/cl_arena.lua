...
function MODE:HUDPaint()
	if zb.ROUND_START + 20 > CurTime() then
		...
		if IsValid(dmmusic) then
			...
			local maxVolume = ply:Alive() and organism.otrub and 0.1 or 1
			local vol = math.Clamp((CurTime() - (zb.ROUND_START + 22)), 0.1, maxVolume)
			if roundend then
				local endMaxVolume = ply:Alive() and organism.otrub and 0 or 1
				vol = math.Clamp((roundend - CurTime() + 1) / 2, 0, endMaxVolume)
			end
    ...
    draw.SimpleText(T("mode_name_slug_arena", "Slug Arena"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local Rolename = T("role_slugcat", slugcat.name)
	local ColorRole = slugcat.color1
    ColorRole.a = 255 * fade
    draw.SimpleText(T("common_your_role_spaced", "Your role: ") .. Rolename, "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.5, ColorRole, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

    local Objective = T("mode_slug_arena_objective", slugcat.objective)
...
CreateEndMenu = function()
    ...
    hmcdEndMenu.Paint = function(self,w,h)
		BlurBackground(self)
		local txt = (wonply and wonply:GetPlayerName() or "아무도").." 승리!"
