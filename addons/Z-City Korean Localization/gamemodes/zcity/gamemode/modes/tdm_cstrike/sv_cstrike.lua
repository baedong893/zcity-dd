...
function MODE:EndRound()
    ...
    zb.RoundsLeft = math.max(zb.RoundsLeft - 1, 0)
    ...
    local seriesComplete = zb.RoundsLeft <= 0
    if not seriesComplete and zb.nextround == self.name then return end

    if seriesComplete then
        local team0Wins = zb.Winners[0] or 0
        local team1Wins = zb.Winners[1] or 0
        local gameWinner = team0Wins > team1Wins and 0 or (team1Wins > team0Wins and 1 or nil)
        local gameWinnerPrint = gameWinner == 1 and "Counter-Terrorists"
            or (gameWinner == 0 and "Terrorists" or "Nobody")

        PrintMessage(HUD_PRINTTALK, gameWinnerPrint.." have won the game.")
    end
