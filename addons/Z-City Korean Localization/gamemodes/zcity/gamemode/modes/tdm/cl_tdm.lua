...
function MODE:HUDPaint()
	...
	if StartTime + 20 > CurTime() then
        ...
        draw.SimpleText( T("mode_tdm_shop_hint", "Press F3 to open the shop"), "ZB_HomicideMedium", sw * 0.5, sh * 0.9, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    else
    ...
    draw.SimpleText("ZBattle | " .. T("mode_name_tdm", "Team Deathmatch"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local roleKey = team_ == 1 and "role_counter_terrorist" or "role_terrorist"
    local Rolename = T(roleKey, teams[team_].name)
    local ColorRole = teams[team_].color1
    ColorRole.a = 255 * fade
    draw.SimpleText(T("common_your_role", "Your role: ") .. Rolename, "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.5, ColorRole, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
CreateEndMenu = function()
	...
	for i, ply in player.Iterator() do
		...
		local fallbackName = ply:GetPlayerName() or ply:Name() or "퇴장함..."
		...
		but.Paint = function(self,w,h)
			...
			local alive = validPly and ply:Alive()
			local playerName = validPly and (ply:GetPlayerName() or ply:Name() or fallbackName) or fallbackName
			local playerColor = validPly and ply:GetPlayerColor():ToColor() or colSpect2
			local frags = tostring(validPly and ply:Frags() or 0)
			local winStreak = validPly and ply:GetNWInt("ZB_WinStreak", 0) or 0
			...
			surface.DrawText(playerName .. ((validPly and not alive) and " - died" or ""))

			surface.SetFont( "ZB_InterfaceMediumLarge" )
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lengthX, lengthY = surface.GetTextSize(frags)
			local winsText = "연승: " .. winStreak
			local winsLengthX, winsLengthY = surface.GetTextSize(winsText)
			surface.SetTextPos(w - winsLengthX - 95,h/2 - winsLengthY/2)
			surface.DrawText(winsText)
		...
		function but:DoClick()
			if isBot or not steamID64 or steamID64 == "" then chat.AddText(Color(255,0,0), "아니요, 그럴 수 없습니다.") return end
...
local function OpenBuyMenu()
	...
	local StartTime = zb.ROUND_START or CurTime()
	local buyTime = (CurrentRound() and CurrentRound().BuyTime) or MODE.BuyTime or 40
	if not LocalPlayer():Alive() or StartTime + buyTime < CurTime() then return end
    ...
    lbl:SetText("남은 시간: "..string.FormattedTime(StartTime + buyTime - CurTime(), "%02i:%02i:%02i"))
    ...
    function lbl:Think()
        if not LocalPlayer():Alive() or StartTime + buyTime < CurTime() then TDM_OpenedBuyMenu:Remove() end
        self:SetText("남은 시간: "..string.FormattedTime(StartTime + buyTime - CurTime(), "%02i:%02i:%02i"))
    end

    local lbl = vgui.Create("DLabel", Frame)
    lbl:SetText("소지금: $"..LocalPlayer():GetNWInt("TDM_Money",0))
    ...
    function lbl:Think()
        self:SetText("소지금: $"..LocalPlayer():GetNWInt("TDM_Money",0))
    end
