...
function MODE:HUDPaint()
	...
	if startTimer > CurTime() then
		...
		local waitingText = T("common_waiting_players", "Waiting for players: ")
		local w, h = surface.GetTextSize(waitingText)
    ...
    draw.SimpleText(T("mode_name_hmcd", "Homicide") .. " | " .. T("mode_name_coop", "CO-OP"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    local Rolename = (lply.role and lply.role.name) or "Unknown"
    local RoleText = T(ZCLang and ZCLang.RoleTextKeys and ZCLang.RoleTextKeys[Rolename] or "", Rolename)
    local ColorRole = teams[0].color1
    ColorRole.a = 255 * fade
    draw.SimpleText(T("common_your_role_spaced", "Your role: ")..RoleText , "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.5, ColorRole, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

    local Objective = lply.PlayerClassName == "Gordon" and T("objective_lead_rebels", "Lead the rebels to victory!") or T("objective_follow_freeman", "Follow Freeman!")
