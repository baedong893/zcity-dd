...
if not ConVarExists("hg_newspectate") then
    CreateClientConVar("hg_newspectate", "1", true, false, "관전 카메라 전환을 부드럽게 할 수 있습니다.", 0, 1)
end
...
function CurrentRound()
	...
	local modeName = zb.CROUND_MAIN or zb:GetMode(round)
	local mode = modeName and zb.modes[modeName]

	zb.CROUND_MAIN = modeName
	if mode and round and round ~= modeName then
		mode.Type = round
	end
...
hook.Add("HUDPaint","FUCKINGSAMENAMEUSEDINHOOKFUCKME",function()
	...
	local txt = T('spectator_target_player', 'Spectating player: ') .. spect:Name()
	local w, h = surface.GetTextSize(txt)
	surface.SetTextPos(ScrW() / 2 - w / 2, ScrH() / 8 * 7)
	surface.DrawText(txt)
	local txt = T('spectator_character_name', 'In-game name: ') .. spect:GetPlayerName()
	local w, h = surface.GetTextSize(txt)
...
net.Receive("RoundInfo", function()
	...
	zb.CROUND = rnd
	zb.CROUND_MAIN = mainRnd ~= "" and mainRnd or rnd
...
local function ZB_IsGunWeapon(wep)
    if not IsValid(wep) then return false end
    if type(ishgweapon) == "function" and ishgweapon(wep) then return true end

    local primaryAmmo = wep.Primary and wep.Primary.Ammo
    if type(primaryAmmo) == "string" and primaryAmmo ~= "" and primaryAmmo ~= "none" then return true end

    local primaryAmmoType = wep.GetPrimaryAmmoType and wep:GetPrimaryAmmoType()
    if primaryAmmoType and primaryAmmoType >= 0 then return true end

    return false
end

local function ZB_HasActiveEnemyTeam(localTeam)
    local foundEnemyTeam = false
    local foundOwnTeam = false

    for _, ply in player.Iterator() do
        if not IsValid(ply) or not ply:Alive() or ply:Team() == TEAM_SPECTATOR then continue end

        if ply:Team() == localTeam then
            foundOwnTeam = true
        else
            foundEnemyTeam = true
        end

        if foundOwnTeam and foundEnemyTeam then return true end
    end

    return false
end

local function ZB_GetTeamAimTarget(ent)
    if not IsValid(ent) then return nil end
    if ent:IsPlayer() then return ent end
    if hg and hg.RagdollOwner then return hg.RagdollOwner(ent) end
end

hook.Add("HUDPaint", "ZCityDrawTeamAimText", function()
    local ply = LocalPlayer()
    if not IsValid(ply) or not ply:Alive() or ply:Team() == TEAM_SPECTATOR then return end
    if zb.ROUND_STATE ~= 1 then return end
    if not ZB_IsGunWeapon(ply:GetActiveWeapon()) then return end
    if not ZB_HasActiveEnemyTeam(ply:Team()) then return end

    local tr = ply:GetEyeTrace()
    local target = ZB_GetTeamAimTarget(tr.Entity)
    if not IsValid(target) or target == ply or not target:Alive() then return end
    if target:Team() ~= ply:Team() or target:Team() == TEAM_SPECTATOR then return end

    local screenPos = tr.HitPos:ToScreen()
    draw.SimpleTextOutlined("아군", "ZB_InterfaceMediumLarge", screenPos.x, screenPos.y + ScreenScale(10), Color(100, 190, 255, 235), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 220))
end)
...
function GM:ScoreboardShow()
	...
	scoreBoardMenu.PaintOver = function(self,w,h)
		...
		surface.SetTextColor(col.r,col.g,col.b,col.a)
        local playersText = T("common_players", "Players:")
        local lengthX, lengthY = surface.GetTextSize(playersText)
		surface.SetTextPos(w / 4 - lengthX/2,ScreenScale(25))
        surface.DrawText(playersText)

		surface.SetFont( "ZB_InterfaceMediumLarge" )
		surface.SetTextColor(col.r,col.g,col.b,col.a)
        local spectatorsText = T("common_spectators", "Spectators:")
        local lengthX, lengthY = surface.GetTextSize(spectatorsText)
	...
	for i, ply in player.Iterator() do -- надо ??о говно пе?едела??.
		...
		but.Paint = function(self, w, h)
			...
			local bangCharacter = ply:GetNWString("HMCD_BangCharacter", "")
			if bangCharacter ~= "" then
				local characterText = "[" .. T("bang_character_" .. bangCharacter, bangCharacter) .. "]"
				surface.SetFont("ZB_InterfaceMedium")
				surface.SetTextColor(235, 190, 70, 255)
				surface.SetTextPos(25 + lengthX, h / 2 - lengthY / 2)
				surface.DrawText(characterText)
			end
	
			surface.SetFont("ZB_InterfaceMediumLarge")
			surface.SetTextColor(col.r, col.g, col.b, col.a)
			local streakText = T('scoreboard_win_streak', 'Wins: ') .. ply:GetNWInt('ZB_WinStreak', 0)
			local streakLengthX, streakLengthY = surface.GetTextSize(streakText)
			surface.SetTextPos(w - streakLengthX - 95, h / 2 - streakLengthY / 2)
			surface.DrawText(streakText)
...
-- ?ве? о? молнии а ?ам? молни? ? не ?делал skill issue
if CLIENT then
...
/*  -- а к??а?и за?ем зде?? н??, ??о же можно б?ло на клиен?е полно???? ?дела??...
	if CLIENT then
...
concommand.Add("zb_snake", function() -- во? как зде??!
    if snakeGameOpen then
        print("[Snake Game] ?г?а ?же зап??ена!")
        return
    end
    ...
    function frame:OnKeyCodePressed(key) -- ФУ?? ?У? ?епе?? пон??но по?ем? лагае? змейка
    ...
    frame.OnClose = function()
        ...
        snakeGameOpen = false  
        print("[Snake Game] ?г?а зак???а.") -- ?? ????Т??Т
    end
