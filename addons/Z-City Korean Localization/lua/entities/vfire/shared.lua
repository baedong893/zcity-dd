...
if SERVER then
	...
	function ENT:ChangeLife(newLife)
		local maxLife = vFireStateToLife(self.stateDown or vFireMaxLife) or vFireMaxLife or 0
		self.life = math.min(newLife or 0, maxLife)
	end
...
if CLIENT then
	...
	function ENT:Draw()
		...
		-- Hot reloads and previously spawned fireballs can briefly carry an old
		-- or invalid networked state. Never index the fixed render buckets with it.
		local state = math.Clamp(math.floor(tonumber(self.visState) or 1), 1, vFireMaxState)
		local stateLightCalls = lightCalls[state]
		if !stateLightCalls then return end
		self.a = tonumber(self.a) or 1
		self.pVis = self.pVis or util.GetPixelVisibleHandle()
	...
	function ENT:ParticlesThink()
		...
		local state = math.Clamp(math.floor(tonumber(self:GetFireState()) or 1), 1, vFireMaxState)
