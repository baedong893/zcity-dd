	...
	function ENT:StickFire(parent, pos, normal)
		...
		timer.Simple(0, function() -- Time it to avoid changing any sort of collision rules on a collision hook
			...
			if IsValid(fire) then
				...
				-- FireState is a bounded enum (1 = Tiny, vFireMaxState = Inferno).
				-- Large fireballs used to network values above that range, which left
				-- clients without a matching particle/light bucket.
				local carriedState = math.Round(self.feedCarry / 10, 0)
				fire:SetFireState(math.Clamp(carriedState, 1, vFireMaxState))
			end
		end)
