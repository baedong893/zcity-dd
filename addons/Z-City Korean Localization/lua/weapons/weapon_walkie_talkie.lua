...
SWEP.PrintName = "무전기"
SWEP.Instructions = "무전기를 사용하여 4km 반경 내의 다른 사람들과 통신할 수 있습니다. 반드시 동일한 주파수로 설정되어 있어야 합니다."
	...
	if(IsValid(owner))then
		...
		cam.Start3D2D( newPos, newAng, 0.005 )
			...
			if self:GetIsOn() then
				...
				draw.SimpleText(self:GetOwner():IsSpeaking() and "송신 중" or "수신 중", "Walkie-Talkie_Fixed-SmallFont", 0, 40, walkietalkie_clr, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		cam.End3D2D()
	...
	if self:GetIsOn() then
		tbl[#tbl + 1] = {function()
			...
			for i = 1, #self.Frequencies do
				...
				tbl1[#tbl1 + 1] = { function() RunConsoleCommand("hg_walkietalkie_adjust", station - self:GetHudFrequency() ) end, "채널 " .. station .. "MHz" }
				hg.CreateRadialMenu(tbl1)
			end
			return -1
		end, "공용 채널"}
		self:MenuAddAdjuster("채널 변경 010.0 MHz", tbl, 010.0)
		self:MenuAddAdjuster("채널 변경 001.0 MHz", tbl, 001.0)
		self:MenuAddAdjuster("채널 변경 000.1 MHz", tbl, 000.1)
	end

	tbl[#tbl + 1] = {function()
		RunConsoleCommand("+reload")
		timer.Simple(0,function() RunConsoleCommand("-reload") end)
	end, self:GetIsOn() and "무전기 끄기" or "무전기 키기"}
