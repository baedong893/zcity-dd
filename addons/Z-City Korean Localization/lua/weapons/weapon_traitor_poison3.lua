...
SWEP.PrintName = "시안화물 가스 용기"
SWEP.Instructions = "가스를 살포하여 사이토크롬 c에서 산소로의 전자 전달을 방해합니다. 그 결과 전자 전달계가 파괴되어, 세포가 더 이상 에너지를 얻기 위한 유산소 ATP 생성을 할 수 없게 됩니다. 중추 신경계나 심장과 같이 유산소 호흡에 크게 의존하는 조직들이 특히 치명적인 영향을 받습니다."
...
if SERVER then
	...
	hook.Add("Org Think", "poison3",function(owner, org, timeValue)
		...
		if ((org.poison3 + 4) < CurTime()) and owner.Profession == "cook" then
			org.owner:Notify("아몬드 같은 냄새가 나... 아니면 향수 냄새..일지도?", true, "cyanide", 3)
		end

		if (not org.poison3notificate) and ((org.poison3 + 20) < CurTime()) then
			org.poison3notificate = true
			org.owner:Notify("숨쉬기 점점 어려워지는데...", true, "cyanide2", 3)
