...
local CantDoIt = {
    "하지만... 아직 살아야 할 이유가 너무 많아!",
    "난... 난 못 하겠어...",
    "다른 방법이 있을 거야. 이건 아니라고!",
    "난... 도저히 직접 이 짓을 할 순 없어.",
    "내가 도대체 무슨 짓을 하고 있는 거지? 난 못 해."
}
--qol lmao
	...
	SWEP.DrawAmmoMetods = {
		["Default"] = function(self,texture)
			...
			if ammoLongCheck > 4 then
				local text = (
                    (clip > clipsize - (self.OpenBolt and 0 or 1) - 1) and "가득 참" or 
                    (clip <= clipsize and clip > clipsize/1.5 ) and "~ 가득 참" or 
                    (clip <= clipsize/1.5 and clip > clipsize/3.5) and "~ 절반 정도" or 
                    (clip <= clipsize/3.5 and clip != 0 ) and "~ 거의 비어 있음" or 
                    (clip == 0 and "비어 있음")
				)
				coloruse.r = 0
				coloruse.g = 0
...
function SWEP:SetHandPos(noset)
	...
	if (ent ~= ply and ent ~= ply.OldRagdoll and !hg.RagdollCombatInUse(ply)) then
		local fingerBone = ent:LookupBone("ValveBiped.Bip01_L_Finger11")
		local fingerAngles = fingerBone and ent:GetManipulateBoneAngles(fingerBone)
		self.lhandik = self.lhandik and !((hg.KeyDown(ply, IN_FORWARD + IN_BACK) or (fingerAngles and fingerAngles[2] < 0)) and !self.reload and !ply:InVehicle())
	end
	...
	local rh, lh = ent:LookupBone("ValveBiped.Bip01_R_Hand"), ent:LookupBone("ValveBiped.Bip01_L_Hand")
	...
	if self:HasAttachment("grip") and hg.CanUseLeftHand(ply) and self.lhandik then
		...
		if not inf.ShouldtUseLHand then
			...
			local newmat = lh and ent:GetBoneMatrix(lh)
			if not newmat then return end
