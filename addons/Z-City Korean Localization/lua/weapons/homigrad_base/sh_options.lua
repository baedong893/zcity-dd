	...
	concommand.Add("hg_change_ammotype", function(ply, cmd, args)
	    ...
	    if wep and ishgweapon(wep) and (wep:Clip1() == 0 or wep.AllwaysChangeAmmo) and wep:CanUse() and wep.AmmoTypes and wep.AmmoTypes[type_] then
	        ply:ChatPrint("탄종이 다음으로 변경됨: " .. wep.AmmoTypes[type_][1])
...
hg.postures = {
    ...
    [5] = "포인트 슈팅",
    [6] = "엄폐 사격",
    [7] = {"갱스터 스타일", isPistolOnly = true},
    [8] = {"한 손 파지", isPistolOnly = true},
    [9] = "소말리안 슈팅",
}
...
if CLIENT then
	...
	concommand.Add("hg_change_posture", function(ply, cmd, args)
		if not args[1] and not isnumber(args[1]) and not printed then print([[총기 파지 자세 변경:
0 - 일반 파지
1 - 지향 사격
2 - 왼쪽 견착
3 - 하이 레디
