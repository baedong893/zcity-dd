...
local hg_random_atts = ConVarExists("hg_random_atts") and GetConVar("hg_random_atts") or CreateConVar("hg_random_atts", 0, FCVAR_SERVER_CAN_EXECUTE, "무기 생성 시 무작위 부착물 활성화 여부", 0, 1)
...
local hg_attachment_draw_distance = ConVarExists("hg_attachment_draw_distance") and GetConVar("hg_attachment_draw_distance") or CreateClientConVar("hg_attachment_draw_distance", 0, true, nil, "부착물 표시 거리", 0, 4096)
...
if CLIENT then
	...
	CreateMenu = function()
		...
		lbl.Paint = function(self, w, h)
            draw.SimpleText("마우스 좌클릭 - 부착물 장착 | 마우스 우클릭 - 부착물 해제", "ZCity_Tiny", w * 0.5, h * 0.5, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
		...
		function frame:RefreshTbl()
			...
			for k, v in pairs(tblcpy) do
				...
				but:SetText( hg.attachmentslaunguage[v[1]]..(v[2] and " - 장착됨" or "") )
