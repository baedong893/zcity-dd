...
SWEP.Instructions = "마우스 왼쪽 버튼(LMB) - 전투 태세 (주먹 올리기)\nRELOAD - 비전투 태세 (주먹 내리기)\n\n[전투 태세]\nLMB - 타격\nRMB - 방어\n\n[비전투 태세]\nRMB - 물체 들어올리기\nRMB + R - 맥박 확인 (대상의 머리나 손에 사용)\n\n[물체를 들고 있을 때]\nRELOAD - 물체를 공중에 고정\nE - 물체를 공중에 회전"
SWEP.Spawnable = true
	...
	if SERVER then
		...
		elseif IsValid(tr.Entity) and tr.Entity:IsPlayer() then
			...
			if Dist < self.ReachDistance then
				...
				local targetIsCSZombie = round and round.name == "cszombie" and tr.Entity.IsCSZombie
				if not targetIsCSZombie and (owner.organism.superfighter or owner.PlayerClassName == "sc_infiltrator" or (clawClasses[owner.PlayerClassName] and !(tr.Entity.PlayerClassName == "furry" or (tr.Entity.IsBerserk and tr.Entity:IsBerserk()))) or owner:IsBerserk()) then
				...
				if not ply2.noHead and ply2.organism then
					...
					elseif ((bone == "ValveBiped.Bip01_L_Hand") or (bone == "ValveBiped.Bip01_R_Hand") or (bone == "ValveBiped.Bip01_Head1")) then
						...
						if (org.last_heartbeat + 60) > CurTime() then
							ply:ChatPrint("시신에 아직 온기가 남아 있습니다.")
                        else
                            ply:ChatPrint((org.last_heartbeat + 180) < CurTime() and "시신이 방치된 지 꽤 시간이 흘렀습니다." or "시신에 약간의 온기가 남아 있습니다.")
                        end

                        if org.blood < 3500 then
                            //if org.blood < 1000 then
                                //ply:ChatPrint("피부색이 거의 백지장처럼 보입니다.")
                            //else
                                ply:ChatPrint("피부가 창백합니다.")
                            //end
                        end

                        if org.bleed > 0 then
                            ply:ChatPrint("시신에서 피가 " .. ((org.bleed > 10 and "철철 흐르고 있습니다.") or (org.bleed > 5 and "적당히 흐르고 있습니다.") or "조금씩 흐르고 있습니다."))
						end
						...
						if org.bulletwounds > 0 then
							ply:ChatPrint("이 시신에서 " .. org.bulletwounds .. "개의 총상을 발견했습니다.")
                        end

                        if org.stabwounds > 0 then
                            ply:ChatPrint("이 시신에서 " .. org.stabwounds .. "개의 자창을 발견했습니다.") // 28번이나 찌르다니. 그에게 기회조차 주지 않았군, 안 그래?
                        end

                        if org.slashwounds > 0 then
                            ply:ChatPrint("이 시신에서 " .. org.slashwounds .. "개의 베인 상처를 발견했습니다.")
                        end

                        if org.bruises > 0 then
                            ply:ChatPrint("이 시신에서 " .. org.bruises .. "개의 멍 자국을 발견했습니다.")
                        end

                        if org.burns > 0 then
                            ply:ChatPrint("시신에 화상 흔적이 있습니다.")
                        end

                        if org.explosionwounds > 0 then
                            ply:ChatPrint("시신에서 폭발로 인한 외상의 흔적이 보입니다.")
						end
						...
						if (bone == "ValveBiped.Bip01_Head1") then
							...
							if org.isPly and not org.otrub then
								org.owner:ChatPrint("누군가 당신의 반응을 확인했습니다.")
                            end
                        end
                    end
                    ...
                    ply:Notify("이 대상의 생체 신호를 확인할 필요는 없을 것 같습니다.", 10)
				end
			end
		end
				...
				if org and bone == "ValveBiped.Bip01_Spine2" and trace.Hit then
					if self.firstTimePrint then
						if not ply2.noHead then
							ply:ChatPrint("심폐소생술(CPR)을 시작합니다.")
                        else
                            ply:Notify("여기서는 심폐소생술이 도움이 되지 않을 것 같습니다...", 10)
						end
					end
...
local customClassInfo = {
	["sc_infiltrator"] = {
		...
		Instructions = "베타 테스트 테스트 베타"
	},
	["furry"] = {
		PrintName = "발바닥",
		WepSelectIcon = Material("zbattle/paw_hmcd.png"),
		handsDesc = "furry",
		Instructions = "마우스 좌클릭 - 앞발 들기\n장전(RELOAD) - 앞발 내리기\n\n앞발을 든 상태:\n마우스 좌클릭 - 타격\n마우스 우클릭 - 방어\n\n<color=91,121,229>퍼슈트 바이러스(Pathowogen) 감염자로서, 당신은 새로운 능력을 갖게 되었습니다.\n\n발을 내린 상태에서, 마우스 우클릭을 유지하여 감염되지 않은 먹잇감을 잡고, 마우스 좌클릭을 유지하여 그들을 감염시키십시오.\n\n마우스 좌클릭으로 동료를 핥아줄 수 있으며, 이는 그들의 고통을 완화하는 데 도움이 됩니다.\n\n:3<color=180,180,180>"
	},
	["headcrabzombie"] = {
		PrintName = "발톱",
		WepSelectIcon = Material("vgui/wep_jack_hmcd_zombhands"),
		handsDesc = "zombie",
		Instructions = "마우스 좌클릭 - 타격\n마우스 우클릭 - 방어/플레이어 잡기\n\n<color=150,0,0>당신의 좀비화된 손입니다. 에너지 소드급은 아니지만, 여전히 강력한 한 방이 있습니다."
	}
}
