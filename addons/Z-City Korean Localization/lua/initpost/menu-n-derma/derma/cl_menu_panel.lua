...
local Selects = {
    {Id = "disconnect", TitleKey = "main_disconnect", Title = "Disconnect", Func = function(luaMenu) RunConsoleCommand("disconnect") end},
    {Id = "main_menu", TitleKey = "main_game_menu", Title = "Main Menu", Func = function(luaMenu) gui.ActivateGameUI() luaMenu:Close() end},
    {Id = "discord", TitleKey = "main_discord", Title = "Discord", Func = function(luaMenu) luaMenu:Close() gui.OpenURL("https://discord.gg/5XPkmFdKj3")  end},
    {Id = "traitor_role", TitleKey = "main_traitor_role", Title = "Traitor Role",
    GamemodeOnly = true,
    ...
    {Id = "achievements", TitleKey = "main_achievements", Title = "Achievements", Func = function(luaMenu,pp) 
        hg.DrawAchievmentsMenu(pp)
    end},
    {Id = "settings", TitleKey = "main_settings", Title = "Settings", Func = function(luaMenu,pp) 
        hg.DrawSettings(pp) 
    end},
    {Id = "appearance", TitleKey = "main_appearance", Title = "Appearance", Func = function(luaMenu,pp) hg.CreateApperanceMenu(pp) end},
    {Id = "back", TitleKey = "main_back", Title = "Back", Func = function(luaMenu) luaMenu:Close() end},
}
...
function PANEL:Init()
    ...
    for k, v in ipairs(Selects) do
        ...
        self:AddSelect(lDock, L(v.TitleKey, v.Title), v)
    end
...
function PANEL:AddSelect( pParent, strTitle, tbl )
    local selectId = tbl.Id or string.lower(tbl.Title or strTitle)
    ...
    function btn:Think()
        ...
        if (crw ~= targetText) or (curent_panel == selectId) then
            local ntxt = ""
            local will_text = (curent_panel == selectId and selectId ~= "traitor_role") and '[ '..string.upper(strTitle)..' ]' or strTitle
