...
local missingPointerWarnings = {}

local function WarnMissingPointerModel(path)
	if missingPointerWarnings[path] then return end
	missingPointerWarnings[path] = true
	ErrorNoHalt("[wOS-DynaBase] Missing animation pointer model: " .. tostring(path) .. "\n")
end
...
local function WRITE_TO_LOCAL( start, finish, type )
	...
	if not mdl_file then
		WarnMissingPointerModel(base_path)
		return
	end
