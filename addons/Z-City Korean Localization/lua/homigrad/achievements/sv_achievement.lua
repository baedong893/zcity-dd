...
//if !hg.init_ach then
-- 뇌사 (braindeath)
    hg.achievements.CreateAchievementType("brain", 1, 0, "저산소증으로 사망하세요.", "난 분명 살아남을 거야...", nil, false)
    -- 약물 과다복용 (death from drugs)
    hg.achievements.CreateAchievementType("drugs", 1, 0, "마약성 진통제(오피오이드) 과다복용으로 사망하세요.", "과부하", nil, false)
    -- 터미네이터 (TERMINATOR)
    hg.achievements.CreateAchievementType("illbeback", 3, 0, "헤드샷을 맞고도 다시 살아나세요.", "I'll be back", nil, true)
    -- 몰살 (kill everyone)
    hg.achievements.CreateAchievementType("killemall", 1, 0, "배신자로서 모두를 죽이고 라운드에서 승리하세요.\n(서버 인원 9명 이상)", "Kill 'Em All", nil, false)
    -- 러시안 룰렛 (russian roulette)
    hg.achievements.CreateAchievementType("deadlygambling", 10, 0, "한 목숨으로 러시안 룰렛에서 10번 연속 생존하세요.", "목숨을 건 도박", nil, true)
    -- 로보토미 킬 (lobotomized kill)
    hg.achievements.CreateAchievementType("lobotomygaming", 1, 0, "능지처참(뇌 손상) 상태에서 배신자를 처치하세요.", "수소폭탄 vs 로보토미 환자", nil, false)
    -- 핫 포테이토 (hot potato)
    hg.achievements.CreateAchievementType("hotpotato", 1, 0, "배신자가 던진 수류탄을 되돌려줘서 처치하세요.", "폭탄 돌리기", nil, false)
    -- 진정하세요 (please calm down)
    hg.achievements.CreateAchievementType("bking", 1, 0, "그 비행기에서 대체 무슨 일이 있었던 거죠...", "손님, 제발 진정하세요", nil, false)
