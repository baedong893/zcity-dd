...
local translateSymbol = {
    ...
    ["L"] = "W",
    -- 한국어 적용 시 (자음 변환 예시)
    ["ㄹ"] = "ㅇ",      -- '나를' -> '나으', '놀자' -> '노자' 등 부드러운 느낌
    ["나"] = "냐",      -- '나'를 '냐'로 변환하여 고양이 느낌 강조
    ["노"] = "뇨",
}
...
local repeating = {
    ["r"] = true,
    ["R"] = true,
    ["ㄹ"] = true,      -- 한국어의 'ㄹ' 반복(갸르르릉)을 허용하기 위함
}
...
if CLIENT then
	...
	local function CreateNotification(msg, showTimer, clr, noChat)
		...
		table.insert(hg.notifications, {msg, (showTimer or defaultShowTimer), clr or Color(255, 255, 255, 255), noChat == true})
	end

	local function CreateNotificationBerserk(msg, showTimer, clr, noChat)
		if hg_furcity:GetBool() or lply.PlayerClassName == "furry" then
		...
		if tbl and clr and tbl[1] and not tbl[5] then
