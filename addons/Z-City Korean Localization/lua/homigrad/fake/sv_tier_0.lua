...
local function CSZombieBlocksFake(ply)
	local round = CurrentRound and CurrentRound()
	return round and round.name == "cszombie" and IsValid(ply) and ply:IsPlayer() and ply.IsCSZombie
end

local function SCP106BlocksFake(ply)
	local round = CurrentRound and CurrentRound()
	return round and round.name == "scp106raid" and IsValid(ply) and ply:IsPlayer() and round.saved and ply == round.saved.SCP106
end

local function GetFallbackRagdollPhysics(ragdoll)
	if not IsValid(ragdoll) then return end

	local preferred = ragdoll:GetPhysicsObjectNum(10)
	if IsValid(preferred) then return preferred end
...
function hg.Ragdoll_Create(ply)
	if CSZombieBlocksFake(ply) then return end
	if SCP106BlocksFake(ply) then return end
	...
	local bodyphy = GetFallbackRagdollPhysics(ragdoll)
	if IsValid(bodyphy) then
		bull:SetPos(bodyphy:GetPos()+bodyphy:GetAngles():Right()*7)
	else
		bull:SetPos(ragdoll:WorldSpaceCenter())
	end
