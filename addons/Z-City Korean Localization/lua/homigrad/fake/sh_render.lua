...
--\\ DrawPlayerRagdoll
	...
	function DrawPlayerRagdoll(ent, ply) --// actually not only ragdoll render but player too
		...
		local isSpectatingThisPlayer = !lply:Alive() and lply:GetNWEntity("spect") == ply
		local isFirstPersonSpectator = isSpectatingThisPlayer and lply:GetNWInt("viewmode", 1) == 1

		if isSpectatingThisPlayer then
			local mat = ent:GetBoneMatrix(lkp)
			if mat then
				mat:SetScale(isFirstPersonSpectator and vector_small or vector_full)
		...
		local wawanted = isFirstPersonSpectator and vector_small or ((GetViewEntity() != ply) and !fountains[ent] and (!(hg_firstperson_death:GetBool() and follow == ent)) and vector_full or vector_small)
