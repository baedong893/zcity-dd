...
hook.Add("RagdollEntityCreated", "RagdollFinder", function(ply, ent, key)
	...
	if ragdoll then
		...
		if IsValid(ply) and not IsValid(ply:GetNWEntity("FakeRagdoll")) and not (ply.organism and ply.organism.otrub) then
			ply:SetNoDraw(false)
...
local function GetRenderableFakeRagdoll(ply)
	if not IsValid(ply) or not ply:IsPlayer() then return false end

	local ragdoll = IsValid(ply.FakeRagdoll) and ply.FakeRagdoll or ply:GetNWEntity("FakeRagdoll")
	if not IsValid(ragdoll) or ragdoll:IsDormant() then return end
	if ragdoll:GetPhysicsObjectCount() <= 0 then return end

	return ragdoll
end

local function HasVisibleFakeRagdoll(ply)
	return IsValid(GetRenderableFakeRagdoll(ply))
end

local function HidePlayerModelForFakeRagdoll(ply)
	if not IsValid(ply) then return end

	ply.ZCityHiddenForFakeRagdoll = true
	ply:DrawShadow(false)
	ply:SetNoDraw(true)
	ply:SetRenderMode(RENDERMODE_NONE)
end

local function RestorePlayerModelAfterFakeRagdoll(ply)
	if not IsValid(ply) or not ply.ZCityHiddenForFakeRagdoll then return end
	if HasVisibleFakeRagdoll(ply) then return end

	ply.ZCityHiddenForFakeRagdoll = nil
	ply:SetNoDraw(false)
	ply:SetRenderMode(RENDERMODE_NORMAL)
	ply:DrawShadow(true)
end

hook.Add("PrePlayerDraw", "ZCityHidePlayerModelDuringFakeRagdoll", function(ply)
	if HasVisibleFakeRagdoll(ply) then
		HidePlayerModelForFakeRagdoll(ply)
		return true
	end

	RestorePlayerModelAfterFakeRagdoll(ply)
end)

hook.Add("Think", "ZCityRestorePlayerModelAfterFakeRagdoll", function()
	for _, ply in player.Iterator() do
		RestorePlayerModelAfterFakeRagdoll(ply)
	end
end)

hook.Add("FakeUp", "ZCityRestorePlayerModelAfterFakeUp", function(ply)
