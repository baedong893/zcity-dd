...
if SERVER then
    ...
    net.Receive( "drop_ammo", function( len, ply )
        ...
        if ply:GetAmmoCount(ammotype)-count < 0 then ply:ChatPrint(((math.random(1,100) == 100 or 1) and "아 니드 모어 불렛!!!!" ) or "탄약이 충분하지 않습니다") return end
        if count < 1 then ply:ChatPrint("0개의 탄약은 버릴 수 없습니다") return end
            --if not ammolistent[ammotype] then ply:ChatPrint("유효하지 않은 엔티티 유형...") return end
