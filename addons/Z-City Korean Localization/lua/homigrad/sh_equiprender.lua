...
if CLIENT then
	...
	function DrawArmors(ply, armors, ent)
		...
		islply = ((ply:IsRagdoll() and hg.RagdollOwner(ply)) or ply) == lply and (LocalPlayer():Alive() and (GetViewEntity() == lply) or (LocalPlayer():GetNWInt("viewmode", 1) == 1))
	...
	hook.Add("radialOptions", "equipment", function()
		...
		if not organism.otrub and table.Count(tbl) > 0 and lply:KeyDown(IN_WALK) then
			...
			local newEntry = {equipmentMenu, "장비 버리기"}
	...
	CreateMenu = function()
		...
		lbl.Paint = function(self, w, h)
			draw.SimpleText("마우스 좌클릭 - 장비 버리기", "ZCity_Tiny", w * 0.5, h * 0.5, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	...
	hook.Add("radialOptions", "9NVG", function()
		...
		if ply:Alive() and not organism.otrub then
			...
			end, ply:GetNWBool("NVG_Enabled", false) and "야간 투시경 비활성화" or "야간 투시경 활성화"}
