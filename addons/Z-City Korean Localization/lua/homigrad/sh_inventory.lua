...
if CLIENT then
	...
	OpenInv = function(ent)
		...
		plyMenu.PaintOver = function(self, w, h)
			...
			draw.DrawText("R - 닫기 | LMB - 가져가기 | RMB - 아이템 메뉴", "HomigradFontSmall", w / 2, h - h*0.055 , clr_text, TEXT_ALIGN_CENTER)
		end
		function plyMenu:Think()
			...
			if not ply:Alive() or (organism and organism.otrub) then self:Remove() return end
			if (ent:GetPos() - ply:GetPos()):LengthSqr() > 125^2 then self:Remove() return end
...
--
		...
		local lootTimeMultiplier = ply:GetNWString("HMCD_BangAbilityCharacter", ply:GetNWString("HMCD_BangCharacter", "")) == "vulture_sam" and 0.5 or 1
		local count = 0
		...
		function DScrollPanel:Paint(w, h)
			...
			draw.DrawText((plyMenu.Created + (count + 3) * lootTimeMultiplier) < CurTime() and "" or txt, "ZCity_Small", w / 2, h / 2.8, Color(255,255,255,15), TEXT_ALIGN_CENTER)
		end
		local count2 = 0
		...
		for tab, things in pairs(inv) do
			...
			for k, i in ipairs(keys) do
				...
				button.Created = CurTime() + ((!ent.foundloot[i] and 2 or 0) + count2) * lootTimeMultiplier
				...
				button.DoClick = function()
					...
					if not functions[tab](ply, ent, i, unpack(thing1)) then
						local OptionsMenu = DermaMenu() 
							OptionsMenu:AddOption( "이미 같은 종류의 아이템을 가지고 있습니다", function() end )
						OptionsMenu:Open()
						return
					end
				...
				button.DoRightClick = function()
					...
					--button:Remove()
					local OptionsMenu = DermaMenu() 
						OptionsMenu:AddOption( "가져가기", function() button:Remove() TakeItem(tab, i, thing, ent) end )
					OptionsMenu:Open()
					--timer.Simple(0.5 * math.max(ply:Ping() / 50,1),function()
					--	--OpenInv(ent)
					--end)
