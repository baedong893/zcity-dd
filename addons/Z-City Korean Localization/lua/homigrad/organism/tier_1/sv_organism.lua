...
hook.Add("Org Clear", "Main", function(org)
	-- A new life must not inherit either Source's FL_GODMODE flag or the
	-- organism damage bypass from a previous round/ability/admin command.
	org.godmode = false
	if IsValid(org.owner) and org.owner:IsPlayer() then
		org.owner:GodDisable()
	end
...
hook.Add("Org Think", "Main", function(owner, org, timeValue)
	...
	if isPly and round and round.name == "cszombie" and owner.IsCSZombie and round.StabilizeZombieOrganism then
		round:StabilizeZombieOrganism(owner)
	end
