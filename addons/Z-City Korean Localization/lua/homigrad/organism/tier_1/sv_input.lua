...
hook.Add("EntityTakeDamage", "homigrad-damage", function(ent, dmgInfo)
	...
	if ply and ply.IsCSZombie and currentRound and currentRound.name == "cszombie" then
		...
		if IsValid(attacker) and attacker:IsPlayer() and attacker ~= ply then
			attacker.CSZombieDamageDealt = (attacker.CSZombieDamageDealt or 0) + math.max(damage, 0)
		end

		if dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT) and IsValid(attacker) and attacker:IsPlayer() and not attacker.IsCSZombie then
			ply.CSZombieStunUntil = CurTime() + 0.3
			ply.CSZombieStunAttacker = attacker

			local knockDir = dmgInfo:GetDamageForce()
			if knockDir:LengthSqr() <= 0 then
				knockDir = ply:GetPos() - attacker:GetShootPos()
			end

			knockDir.z = 0
			if knockDir:LengthSqr() > 0 then
				knockDir:Normalize()
				local knockPower = math.Clamp(16 + damage * 0.55, 16, 34)
				ply:SetVelocity(knockDir * knockPower + Vector(0, 0, 4))
			end
		end
		...
		if currentRound.ResetZombieOrganismDamage then
			currentRound:ResetZombieOrganismDamage(ply)
		end
	...
	if dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT + DMG_BLAST + DMG_SLASH + DMG_CLUB + DMG_GENERIC) then
		...
		if ply then
			...
			local preventRoundBulletFall = round and round.name == "cszombie" and ply.IsCSZombie and dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT)
			...
			if not preventRoundBulletFall then
