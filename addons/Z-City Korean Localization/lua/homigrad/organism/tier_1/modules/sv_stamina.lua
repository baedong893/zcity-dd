...
local hg_infstamina = CreateConVar("hg_infstamina", "0", FCVAR_ARCHIVE + FCVAR_NOTIFY, "무한 스테미나 설정 (달리기/공격 시 소모 안 됨, 다른 상태 이상 효과로만 소모)", 0, 1)
module[2] = function(owner, org, timeValue)
    ...
    if painfrommoving > 0 then
        //org.owner:Notify("움직임을 좀 줄여야겠어... 너무 아파.", 30, "painfrommoving", 0, nil, Color(255, 0, 0))
    
        if (org.jaw == 1) or org.jawdislocation then
            //org.owner:Notify("움직일 때마다 턱이 너무 울려.", 60, "painfromjaw", 0, nil, Color(255, 210, 210))
        end

        if (org.chest > 0.25) then
            //org.owner:Notify("숨쉬기가 고통스러워. 내 갈비뼈에 문제가 생긴 게 분명해.", 60, "painfromribs", 0, nil, Color(255, 210, 210))
        end
    end
