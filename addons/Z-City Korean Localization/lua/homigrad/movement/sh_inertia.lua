...
--\\ Inertia & stuff
	...
	local function isCSZombieZombie(ply)
		local round = CurrentRound and CurrentRound()
		return round and round.name == "cszombie" and IsValid(ply) and (ply.IsCSZombie or ply:GetNWBool("CSZombie_IsZombie", false))
	end
	...
	hook.Add("SetupMove", "HG(StartCommand)", function(ply, mv, cmd)
		...
		k = 1 * weightmul
		local cszombieZombie = isCSZombieZombie(ply)
		if not cszombieZombie then
		...
		if not cszombieZombie and (slwdwn > 0) then
			//if(SERVER)then
		...
		if not cszombieZombie and ply:GetNetVar("vomiting", 0) > (CurTime() - 3) then
			k = k * 0.25
		end
