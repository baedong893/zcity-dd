...
function PANEL:PerformLayout(width, height)
	...
	self:SetTall(math.max(self.markup:GetHeight(), (self.ZChatLinkButtonCount or 0) * 20 + 2))
end
...
local function TrimUrl(url)
	while url:sub(-1) == "." or url:sub(-1) == "," or url:sub(-1) == ")" or url:sub(-1) == "]" do
		url = url:sub(1, -2)
	end
...
local function NormalizeUrl(url)
	...
	if url:sub(1, 4) == "www." or url:sub(1, 11) == "discord.gg/" then
		return "https://" .. url
	end
...
local function ExtractUrlsFromElements(elements)
	local urls = {}
	local seen = {}
	...
	for _, v in ipairs(elements) do
		...
		for url in text:gmatch("www%.[%w%p]+") do
			url = NormalizeUrl(url)
			if not seen[url] then
				seen[url] = true
				urls[#urls + 1] = url
		...
		for url in text:gmatch("discord%.gg/[%w%p]+") do
			url = NormalizeUrl(url)
			if not seen[url] then
				seen[url] = true
				urls[#urls + 1] = url
...
local function AddLinkButtons(panel, urls)
	if #urls <= 0 then return end

	panel:SetMouseInputEnabled(true)
	panel.ZChatLinkButtons = panel.ZChatLinkButtons or {}

	for i, url in ipairs(urls) do
		local button = panel:Add("DButton")
		button:SetText(#urls > 1 and ("URL " .. i) or "URL")
		button:SetTooltip(url)
		button:SetSize(54, 18)
		button:SetPos(math.max(panel:GetWide() - 56, 0), 2 + (i - 1) * 20)
		button:SetZPos(10)

		function button:Think()
			self:SetPos(math.max(panel:GetWide() - 56, 0), 2 + (i - 1) * 20)

			local chat = hg and hg.chat
			local visible = IsValid(chat) and chat:GetActive() and panel:IsVisible()
			self:SetVisible(visible)
			self:SetMouseInputEnabled(visible)
		end
		...
		function button:Paint(w, h)
			surface.SetDrawColor(25, 75, 140, 210)
			surface.DrawRect(0, 0, w, h)
			surface.SetDrawColor(120, 175, 255, 230)
			surface.DrawOutlinedRect(0, 0, w, h, 1)
			draw.SimpleText(self:GetText(), "zChatFontSmall", w * 0.5, h * 0.5, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end

		panel.ZChatLinkButtons[#panel.ZChatLinkButtons + 1] = button
	end
end
...
function PANEL:SetActive(bActive, bRemovePrev)
	...
	for _, entry in ipairs(self.entries or {}) do
		if not IsValid(entry) or not entry.ZChatLinkButtons then continue end

		for _, button in ipairs(entry.ZChatLinkButtons) do
			if IsValid(button) then
				button:SetVisible(bActive)
				button:SetMouseInputEnabled(bActive)
			end
		end
	end
...
function PANEL:AddLine(elements)
	...
	local urls = ExtractUrlsFromElements(elements)
	...
	panel.ZChatLinkButtonCount = #urls
	panel:SetMarkup(table.concat(buffer))
	AddLinkButtons(panel, urls)
