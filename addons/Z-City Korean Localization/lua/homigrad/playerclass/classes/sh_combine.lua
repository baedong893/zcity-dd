...
local callsigns = {
    "알파", "브라보", "찰리", "델타", "에코",
    "폭스트롯", "탱고", "시에라", "유니폼", "킬로", "양키", "리젠트",
    "줄루", "오메가", "노바", "바이퍼", "고스트", "레이븐",
}

local leader_callsigns = {"리더", "오블리터레이터", "오버로드", "디렉터", "컨트롤"}
...
function CLASS.On(self, data)
    ...
    if zb.GiveRole then zb.GiveRole(self, self.leader and "분대장" or "병사", Color(89,230,255)) end
...
if CLIENT then
    ...
    function CLASS.HUDPaint(self)
        ...
        do
            ...
            draw.DrawText("% | 혈액량","CMBFontSmall",
        ...
        do
            ...
            draw.DrawText("| 분당 심박수","CMBFontSmall",
        ...
        do
            ...
            if round and round.name == "cszombie" then
                stamina_txt = 0
            else
            ...
            draw.DrawText("| 스테미나","CMBFontSmall",
