...
elseif SERVER then
    ...
    local phrases = {
        "고든 프리맨이 사망했습니다. 이제 어떻게 되는 거죠?",
        "그가 죽었습니다. 이제 누가 길을 인도하죠?",
        "고든 프리맨이 임무 수행에 실패했습니다.",
        "참으로 유능한 직원의 안타까운 결말이군요."
    }
    ...
    hook.Add("Org Think","gordon_healing",function(ply, org, timeValue)
        ...
        if ply.PlayerClassName == "Gordon" and ply:GetNetVar("HEVSuit") then
            ...
            if org.brain > 0.1 then
                ...
                ply:Notify("HEV 슈트가 외상성 뇌 손상을 감지했습니다. 만니톨을 주입합니다.",true,"mannitol_hev",0.5,function(ply)
            ...
            if org.pneumothorax > 0 then
                ...
                ply:Notify("HEV 슈트가 기흉을 감지했습니다. 복구 중입니다.", true, "needle_hev", 0.5, function(ply)
                    net.Start("HEV_DAMAGE")
                        net.WriteString("hl1/fvox/automedic_on.wav")
                    net.Send(ply)
                end, hev_color)
            ...
            if pain > 30 then
                if (org.NextMorphineInject or 0) < CurTime() then
                    ...
                    if administer > 0.1 then
                        ply:Notify("HEV 슈트가 통증 수용체의 한계치 도달을 감지했습니다. 모르핀을 주입합니다.", 10, "morphine_hev", 0.5,
                        function(ply)
                            net.Start("HEV_DAMAGE")
            ...
            if (org.CO > 10) or (org.COregen > 10) then
                ply:Notify("HEV 슈트가 체내 일산화탄소 존재를 감지했습니다. 중화 중입니다.",60,"co_hev",0.5,function(ply)
                    net.Start("HEV_DAMAGE")
                        net.WriteString("hl1/fvox/automedic_on.wav")
                    net.Send(ply)
                end, hev_color)
            ...
            if (org.BoneCheck or 0) < CurTime() then
                ...
                if bonesfixed then
                    ply:Notify("HEV 슈트가 골절을 감지했습니다. 복구 중입니다.",60,"bones_hev",0.5,function(ply)
                        net.Start("HEV_DAMAGE")
                            net.WriteString("hl1/fvox/automedic_on.wav")
                        net.Send(ply)
                    end, hev_color)
            ...
            if (org.pulse < 40) or (org.blood < 3000) or (org.o2[1] < 10) then
                ply:Notify("HEV 슈트가 극도로 낮은 맥박을 감지했습니다. 에피네프린을 주입했습니다. 자동 맥박 보조를 활성화합니다. 혈장을 주입했습니다.", 60, "pulse_hev", 0.5, function(ply)
                    net.Start("HEV_DAMAGE")
