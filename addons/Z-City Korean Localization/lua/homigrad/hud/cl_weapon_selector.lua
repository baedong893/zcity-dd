...
function WS.GetSelectedWeapon()
    ...
    local selectedSlot = Weapons[WS.SelectedSlot]
    local lastSelectedSlot = Weapons[WS.LastSelectedSlot]
    local fallbackSlot = Weapons[0]

    return (selectedSlot and selectedSlot[WS.SelectedSlotPos])
        or (lastSelectedSlot and lastSelectedSlot[WS.LastSelectedSlotPos])
        or (fallbackSlot and fallbackSlot[0])
        or ply:GetActiveWeapon()
end
...
local function GetUpper(Weapons)
    ...
    for _ = 0, #Weapons do
        ...
        if Weapons[WS.SelectedSlot] and Weapons[WS.SelectedSlot][WS.SelectedSlotPos] then
            return
        end
    end
end
...
function WS.ChangeSelectionWep( ply, key )
    ...
    if iPos or key == "invnext" or key == "invprev" or key == "lastinv" then
		...
		elseif key == "lastinv" then
			local lastWeapon = WS.LastInv
			if IsValid(lastWeapon) and lastWeapon:GetOwner() == ply then
				local activeWeapon = ply:GetActiveWeapon()
				WS.Show = 0
				input.SelectWeapon(lastWeapon)
				WS.LastInv = IsValid(activeWeapon) and activeWeapon or nil
			end
        end
    ...
    if (cmd:KeyDown( IN_ATTACK ) or cmd:KeyDown( IN_ATTACK2 )) and WS.Show > CurTime() then
        ...
        if WS.Selected and WS.Selected > CurTime() then 
			...
			if IsValid(selectedWeapon) then
				local activeWeapon = ply:GetActiveWeapon()
				if selectedWeapon ~= activeWeapon and IsValid(activeWeapon) then
					WS.LastInv = activeWeapon
				end
