    ...
    if ply:Alive() and not organism.otrub and hg.GetCurrentCharacter(ply) == ply then
        ...
        local tbl = {function(mouseClick)
            if mouseClick == 1 then
                -- 좌클릭: 랜덤 실행 (영어 명령어 전송)
                local rand = randomGestures[math.random(#randomGestures)]
                local cmd = istable(rand) and rand[1] or rand
                RunConsoleCommand("act", cmd)
                ...
                for i, str in ipairs(randomGestures) do
                    ...
                    commands[i] = {
                        ...
                        -- [핵심] 화면 표시 이름만 한글로 교체 (없으면 원래 이름 사용)
                        [2] = gestureNames[rawName] or string.NiceName(rawName)
                    }
                end
