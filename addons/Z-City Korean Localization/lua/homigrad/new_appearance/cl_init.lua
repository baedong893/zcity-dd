...
function hg.Appearance.LoadAppearanceFile(strFile_name)
	...
	if not hg.Appearance.AppearanceValidater(tblAppearance) then return false, "외형 파일이 올바르지 않습니다: data/zcity/appearances/" .. strFile_name .. ".json"  end
...
net.Receive("Get_Appearance", function()
    ...
    if not tbl and not forced_random and reason then print("[Appearance] load failed - " .. tostring(reason)) end
end)
...
local function IsFirstPersonRenderTarget(ply)
	local localPly = LocalPlayer()
	if not IsValid(localPly) or not IsValid(ply) then return false end

	local owner = (ply:IsRagdoll() and hg.RagdollOwner(ply)) or ply
	local target = localPly:Alive() and localPly or localPly:GetNWEntity("spect", localPly)
	if owner ~= target then return false end

	if localPly:Alive() then
		return GetViewEntity() == target
	end

	return localPly:GetNWInt("viewmode", 1) == 1
end
...
function RenderAccessories(ply, accessories, setup)
	...
	islply = IsFirstPersonRenderTarget(ply)
