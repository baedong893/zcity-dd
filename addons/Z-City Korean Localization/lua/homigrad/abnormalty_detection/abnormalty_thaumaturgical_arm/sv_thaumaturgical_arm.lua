...
local function TryConjureTA(zone, ply)
	...
	if(PLUGIN.GetZoneOrPlyEqualizers(zone, ply) >= equalizers_consumption and PLUGIN.GetZoneOrPlyBlood(zone, ply) >= blood_consumption)then
		...
		if(ent)then
			PLUGIN.ShowMessageInSphere("Thaumaturgic Arm 소환 중...", zone.Pos, zone.Radius)
			...
			PLUGIN.ShowMessage(ply, "구역 내에 제물로 바칠 근접 무기가 없습니다.")
		end
	else
		PLUGIN.ShowMessage(ply, "Thaumaturgic Arm을 소환하기 위한 Equalizers 혹은 혈액이 부족합니다.")
	end
end
--//
