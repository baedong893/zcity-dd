...
local function Ressurect(owner, body, time)
	PLUGIN.ShowMessageToAll("지금 무언가 일어나고 있습니다")
	...
	if(PLUGIN.GetZoneOrPlyBlood(zone, ply) >= blood_consumption)then
		...
		if(body)then
			...
			if(!PLUGIN.Ressurection.ToRessurect[owner])then
				PLUGIN.ShowMessageInSphere(body:GetNWString("PlayerName") .. "을(를) 되살리는 중...", zone.Pos, zone.Radius)
				...
				PLUGIN.ShowMessage(ply, "구역 내에 시신이 존재하지 않습니다.")
			end
		...
		PLUGIN.ShowMessage(ply, "육체를 재기동하기 위한 혈액이 부족합니다.")
	end
end
--//
	...
	for ply, info in pairs(PLUGIN.Ressurection.ToRessurect) do
		if(info.Time <= CurTime())then
			...
			if(!IsValid(owner) or owner:Alive() or !IsValid(body))then
				PLUGIN.ShowMessageToAll("신성한 개입에 의해 의식이 중단되었습니다.\n오염이 확산됩니다.")
			else
				...
				PLUGIN.ShowMessageToAll("사악한 무언가가 벌어졌습니다")
			end
