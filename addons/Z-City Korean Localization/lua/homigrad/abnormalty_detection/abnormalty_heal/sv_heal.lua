...
local function TryHeal(zone, ply)
	...
	if(PLUGIN.GetZoneOrPlyBlood(zone, ply) >= blood_consumption)then
		...
		if(owner and !PLUGIN.Heal.ToHeal[owner])then
			Heal(owner, 5)
			PLUGIN.ShowMessageInSphere(owner:GetNWString("PlayerName") .. "님을 치유 중...", zone.Pos, zone.Radius)
			...
			PLUGIN.ShowMessage(ply, "구역 내에 치유할 대상이 없습니다.")
		end
	else
		PLUGIN.ShowMessage(ply, "치유를 진행하기 위한 혈액이 부족합니다.")
	end
end
--//
