...
local function TryInvis(zone, ply)
	...
	if(PLUGIN.GetZoneOrPlyEqualizers(zone, ply) >= consumption)then
		...
		if(owner and !PLUGIN.Invisibility.ToInvis[owner])then
			PLUGIN.ShowMessageInSphere(owner:GetNWString("PlayerName") .. "의 Cogito 통신을 차단 중...", zone.Pos, zone.Radius)
			...
			PLUGIN.ShowMessage(ply, "구역 내에 대상이 존재하지 않습니다.")
		end
	else
		PLUGIN.ShowMessage(ply, "은신 처리를 위한 Equalizers가 부족합니다.")
	end
end
--//
